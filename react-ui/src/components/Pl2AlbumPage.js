  ...
  useEffect(() => {
    ...
    const getUserTracksByAlbum = async () => {
      const response = await fetch(
        `${API_URL}/tracks/user/albums/${props.currentUser.id}`,
        {
          method: "GET",
          mode: "cors",
              ...
              <div ke={1000 + kii} className={"pl2-album-c"}>
                ...
                <div className={"pl2-album-info-c"}>
                  ...
                  <h2 className={"pl2-artist-info-c__top__h2"}>
                    {album[0].trackalbum.length > 30
                      ? `${album[0].trackalbum.slice(0, 30)}...`
                      : album[0].trackalbum}
                  </h2>
                  <h2 className={"pl2-artist-info-c__top__h2-450"}>
                    {album[0].trackalbum.length > 15
                      ? `${album[0].trackalbum.slice(0, 15)}...`
                      : album[0].trackalbum}
                  </h2>
