...
const TrackRatingModal = (props) => {
  ...
  function rateTrack() {
    trackRatingInput.current.value = emojiSelect.current.value.repeat(
      ratingNumberSelect.current.value
    );
  }
  ...
  return ReactDom.createPortal(
    <div>
      <div
        id={"track-rating-modal-background"}
        onClick={() => {
          props.setTrackRatingModalState(false);
        }}
      ></div>
      <div id={"track-rating-modal-top"}>
        <div
          id={"track-rating-modal-close-button"}
          onClick={() => {
            props.setTrackRatingModalState(false);
          }}
        >
        ...
        <form
          id={"track-rating-form"}
          name={props.ratingAudioState.id}
          onSubmit={(e) => {
            e.preventDefault();
        ...
        >
          <input
            ...
            maxLength={100}
            placeholder={
              props.ratingAudioState.trackrating
                ? props.ratingAudioState.trackrating
                : ""
            }
          />
          <span className={"track-rating-text"}>
            pick a number and a emoji to rate your track
          </span>
          <input id={"pl2-track-genre-input-submit"} type={"submit"} />
        </form>
        <div id={"select-box"}>
          <input
            ref={ratingNumberSelect}
